...
const translations: Record<Language, Record<string, string>> = {
  ...
  zh: {
    ...
    'blog.grpc.howItWorksText':
      '这通常是在 .proto 文件中使用协议缓冲区 (Protobuf) 完成的。在高层次上，gRPC 允许你定义服务，指定可以远程调用的方法及其参数和返回类型。服务器实现此接口并运行 gRPC 服务器来处理传入的调用。客户端有一个存根（在某些语言中简称为客户端），它提供与服务器相同的方法。默认情况下，gRPC 使用协议缓冲区（Protobuf）作为其接口定义语言（IDL）和底层消息交换格式。在 REST 中，你通常使用 JSON。使用 Protobuf，你在 .proto 文件中定义一次数据结构。然后，你使用 protoc 编译器在你喜欢的编程语言中生成数据访问类。这种二进制格式比 JSON 更轻便，序列化/反序列化速度更快。',
